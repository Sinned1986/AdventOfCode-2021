...
# generates arrays for all bingo boards, we want to mark called numbers in those arrays
...
def day04a(plot_board=False):
    ...
    try:
        for game_round, called_number in enumerate(bs):
            for player, board in enumerate(bbs):
                search_result = np.where(board == called_number)
                if len(search_result[0]) == 1:
                    bbs_h[player][search_result[0], search_result[1]] = True
                    cross_h = bbs_h[player][search_result[0], :]
                    cross_v = bbs_h[player][:, search_result[1]]
                    if np.all(cross_h):
                        winner_numbers = np.reshape(board[search_result[0], :], -1)
                        raise EndGameException(player, game_round, winner_numbers)
                    if np.all(cross_v):
